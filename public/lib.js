...
(() => {
  ...
  const buildTaskPrompt = ({ grade = null, gradeLabel = '', topic = null, subtopics = [], subtopic = null, count = 30, tags = [] } = {}) => {
    ...
    push('  Несколько величин — через запятую, каждая со своим именем: «$AD = 4\\sqrt{3}\\text{ см}$, $BC = 6\\sqrt{3}\\text{ см}$». Сайт даст по полю на величину.');
    push('  Несколько значений одной величины — через «;» по возрастанию: «$x_1 = -2;\\; x_2 = 5$», «$-9; -2; 0; 4; 7$».');
    push('  Точка — «$(3; 8)$» или «$B(3; 4)$», промежуток — «$x \\in (-3; 5]$»: сайт разложит их на поля по координатам и границам.');
    push('  Слов в ответе быть не должно, кроме единицы измерения в \\text{}. Если ответ словесный по сути («Да», «Доказано»), так и напиши, но тогда задача уйдёт на самопроверку.');
    push('- hint_latex / hint_latex_lv — подсказка: какой приём или теорему применить. Ответа и промежуточных чисел в ней нет.');
    push('  У задачи в два действия и больше подсказка — 2–3 шага, каждый про один приём: первый — с чего начать, следующий — куда двигаться дальше. Шаги разделяй строкой из трёх дефисов --- (ячейка в кавычках, как у решения). Ученик открывает шаги по одному. Ответа нет ни в одном шаге.');
  ...
  /* Подсказка из нескольких шагов, как у Khan Academy: каждый следующий
     шаг открывается отдельно, и ученик берёт ровно столько помощи, сколько
     нужно. Шаги пишутся в том же поле, что и раньше, через строку «---»:
     импорт, выгрузка и резервные копии остаются текстом, а старая подсказка
     без разделителя — это просто один шаг. */
  const HINT_STEP_SEPARATOR = /\r?\n[ \t]*-{3,}[ \t]*(?=\r?\n|$)/;
  const splitHintSteps = text => String(text || '')
    .split(HINT_STEP_SEPARATOR)
    .map(step => step.trim())
    .filter(Boolean);

  /* Шаги, которые уже называют ответ. Последний шаг вправе подвести к
     ответу вплотную — ради него ученик и прошёл цепочку, — а ранний шаг с
     ответом делает остальные бессмысленными. */
  const hintStepsRevealAnswer = (answer, hint) => {
    const steps = splitHintSteps(hint);
    if (steps.length <= 1) return hintRevealsAnswer(answer, hint) ? [0] : [];
    return steps
      .map((step, index) => (index < steps.length - 1 && hintRevealsAnswer(answer, step) ? index : -1))
