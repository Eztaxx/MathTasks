...
1. **Перенос ключа в секреты хостинга:**
   - Для Cloudflare Workers: `npx wrangler secret put GEMINI_API_KEY` или через Cloudflare Dashboard.
2. **Активация эндпоинта `/api/generate-task` в [`worker/index.js`](file:///C:/Users/goga8/Downloads/MathTasks/worker/index.js):**
...
## 4. 📌 Состояние на 6 сентября 2026 года (архивный срез, устарело)

> [!NOTE]
> Данный раздел фиксирует начальное историческое состояние на 6 сентября 2026 года и устарел:
> - В репозитории уже добавлена 41 SVG-иллюстрация к условиям задач (`task-svgs/condition/`), синхронизируемая с Supabase Storage (`task-images`) через команду `npm run sync:svgs`.
> - По снимку базы от 13 сентября 2026 года (`docs/backup/snapshots/`): 552 задачи, из них 542 опубликованы, 136 тем, у 41 задачи есть чертёж.
> - Часть предложений из раздела 5 уже реализована в кодовой базе и миграциях (подсказки к задачам `hint_latex`, объединение разделов Skola2030, прокси к Gemini в `worker/index.js`).

Чтобы этот документ не расходился с реальностью, здесь зафиксировано то, что уже сделано, и то, что мешало двигаться дальше на момент 6 сентября 2026 года. Цифры — из боевой базы на ту дату.
...
| Задач с чертежом | 0 *(исторически; в репозитории уже 41 чертёж в `task-svgs/condition/`)* |
...
**Чертежи к геометрии.** *(В процессе/реализовано: в репозиторий добавлена 41 SVG-иллюстрация в `task-svgs/condition/` и скрипт синхронизации `npm run sync:svgs`)*. Загрузка картинок работает. Геометрия за 7–9 класс без чертежей неполноценна — это не украшение, а часть условия.

**Заполнить пустые разделы.** Арифметика, Функции, Тригонометрия, Стереометрия, Логика, Математический анализ созданы, но пусты: все 125 тем лежат в Алгебре, Планиметрии и Статистике. Либо перераспределить темы, либо удалить лишние разделы — сейчас это недоделанное решение.
...
### 5.4. Ключ Gemini на сервере — сделано

Ключ лежит в секретах воркера (`GEMINI_API_KEY`). Генератор и перевод в админке ходят в `/api/gemini`, поле ввода ключа из панели убрано, а ключ, сохранённый раньше в браузере, стирается при открытии админки.

Воркер пропускает запрос только с токеном входа администратора. Роль проверяет сама база — функцией `is_admin()`, той же, что защищает таблицы (вызов `rpc/is_admin` с токеном пользователя), — поэтому отдельный `supabase.auth.getUser(token)` из раздела 1.3 не понадобился. Лимит — 30 запросов в минуту на человека. Старый `/api/generate-task` без проверки роли удалён.
...
### 5.6. Лиги — эскиз, не сделано

После профилей (миграция 027) можно добавить недельные лиги, как у Duolingo, — но только замкнутые, не глобальные. **Лига по коду**: учитель или ученик создаёт лигу (`leagues(code, title, week_start)` + `league_members(profile_id, league_id)`), участники видны только с ником и только членам лиги (RLS по членству), не больше 30 человек, очки обнуляются каждый понедельник. Очки считает серверная функция из снимка `student_progress.data` — всё равно данные клиентские, но хотя бы через одну дверь. Для дуэлей в лиге подпись уже не нужна: счёт попытки считает воркер (раздел 5.7), лиге остаётся читать `duel_runs`.

### 5.7. Таблица лидеров дуэлей — сделано в feature/learning, не включено

Общая таблица по категориям и сложностям, за неделю и за всё время: у каждого игрока одна лучшая попытка, 20 мест. Сначала мы решили глобальных топов не делать, потом решение пересмотрели. Поэтому таблица сделана так, чтобы подделка стоила дорого, а о ребёнке в ней не было ничего.

**Почему нельзя писать из браузера.** Раньше попытку записывала RPC, открытая всем: скрипт мог прислать любой счёт. Теперь в `duel_runs` пишет только служебная роль, а её ключ хранится только в секретах воркера. Браузер присылает ответы и время каждого ответа. Всё остальное делает `worker/duel-api.js`:

1. Отмечает начало минуты по часам базы (`/api/duel/start`).
2. В конце (`/api/duel/finish`) заново генерирует те же примеры из зерна генераторами тренажёра и сам проверяет каждый ответ.
3. Проверяет правдоподобие попытки (`assessRun` в `public/duel.js`). Отсекаются:
   - счёт выше потолка для этой категории и сложности, это примерно уровень лучших взрослых;
   - ответы чаще, чем раз в 0,25 с;
   - слишком ровный, «метрономный» темп;
   - ответы, пришедшие позже, чем закончилась попытка по часам сервера.
4. Спрашивает Cloudflare Turnstile «не робот ли».
5. Ограничивает частоту запросов с одного адреса: не больше 20 в минуту, привязка `DUEL_LIMIT`.

Попытка, не прошедшая проверку, сохраняется, но не попадает ни в таблицу, ни в соперники-записи. Робота, который подражает человеку, это не остановит совсем, но выше лучших людей он не поднимется. Такую запись администратор скрывает через `duel_hide(id)`.

**Потолки подобраны по оценке.** Когда накопятся настоящие попытки, их нужно сверить с данными: `select cat, diff, max(correct), percentile_cont(0.99) within group (order by correct) from duel_runs where verified group by 1, 2`.

**Что хранится.** Хранятся:
- ник — фильтр в воркере и ещё раз при показе;
- случайный номер игрока из localStorage, чтобы у игрока была одна строка;
- ответы и время.

Ни имени, ни почты, ни IP-адреса. Записи старше 60 дней удаляются, кроме лучших за всё время.

**Как включить:**

1. Выполнить `supabase/migrations/028_duel_runs.sql` в SQL Editor.
2. Задать служебный ключ воркера:

   ```
   npx wrangler secret put SUPABASE_SERVICE_ROLE_KEY
   ```

   Ключ: Supabase → Project Settings → API → service_role. Это уже включит соперников-записей.
3. Cloudflare → Turnstile → Add widget, домен `mathtasks.lv`, режим Managed. Его ключи задать секретами:
...
Пока ключей нет, `/api/duel/config` отвечает «выключено», и дуэли работают без записи.
