...
После профилей (миграция 027) можно добавить недельные лиги, как у Duolingo, — но только замкнутые, не глобальные. **Лига по коду**: учитель или ученик создаёт лигу (`leagues(code, title, week_start)` + `league_members(profile_id, league_id)`), участники видны только с ником и только членам лиги (RLS по членству), не больше 30 человек, очки обнуляются каждый понедельник. Очки считает серверная функция из снимка `student_progress.data` — всё равно данные клиентские, но хотя бы через одну дверь. Для дуэлей в лиге понадобится подпись результата на сервере: HMAC в воркере по образцу `/api/gemini`, секрет `DUEL_SECRET`. Глобальных топов детей не делать: давление без пользы и лишний риск.
