...
/* Уроки — файлы в статике, поэтому их список воркер берёт у ASSETS. */
describe('seo: уроки', () => {
  const INDEX = [{ slug: 'linear-function-k-b', grade: 7, steps: 7,
    title: { ru: 'Линейная функция', lv: 'Lineāra funkcija' },
    description: { ru: 'Ползунки k и b', lv: 'Slīdņi k un b' } }];
  const lessonEnv = () => ({
    SUPABASE_URL: 'https://db.example',
    SUPABASE_KEY: 'anon',
    ASSETS: {
      fetch: vi.fn().mockImplementation(async request => {
        ...
        if (url.includes('/data/lessons/index.json')) return new Response(JSON.stringify(INDEX), { headers: { 'content-type': 'application/json' } });
        return htmlAsset();
      })
    }
  });

  it('адреса уроков разбираются', () => {
    expect(routeOf('/lesson/linear-function-k-b')).toEqual({ kind: 'lesson', slug: 'linear-function-k-b' });
    expect(routeOf('/lv/lesson/linear-function-k-b')).toEqual({ kind: 'lesson', slug: 'linear-function-k-b' });
    expect(routeOf('/lessons')).toEqual({ kind: 'static', path: '/lessons' });
  });

  it('урок: заголовок, описание и путь по каталогу на языке страницы', async () => {
    const ru = await (await page('/lesson/linear-function-k-b', lessonEnv())).text();
    expect(ru).toContain('<title>Линейная функция, 7 класс — MathTasks</title>');
    expect(ru).toContain('<a href="/lessons">Уроки</a>');
    const lv = await (await page('/lv/lesson/linear-function-k-b', lessonEnv())).text();
    expect(lv).toContain('<title>Lineāra funkcija, 7. klase — MathTasks</title>');
  });

  it('неизвестный урок — 404', async () => {
    const response = await page('/lesson/net-takogo', lessonEnv());
    expect(response.status).toBe(404);
  });
});
